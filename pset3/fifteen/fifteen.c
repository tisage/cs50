...
void init(void)
{
    ...
    */
    if((d*d)%2==0){ // actually only swap when d^2 is even number
...
bool move(int tile)
{
    ...
    // locate our tile position according to the value of tile
    int row=0;
    int col=0;
    for(int i=0;i<d;i++){
        for(int j=0;j<d;j++){
    ...
    if(board[row][col+1]==0&&(col+1)<d){ // right side, remember to check boarder issue col + 1 < d
        board[row][col+1]=board[row][col]; // swap
        board[row][col]=0;
        return true;
    }
...
{
    // TODO
    // User a counter and compare it with board[i][j]
